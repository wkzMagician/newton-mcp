...
class ModelBuilder:
    ...
    def set_body_velocity(self, body: int, vel: wp.spatial_vector):
        """Sets the linear and angular velocity of a body.

        Args:
            body: The index of the body to set the velocity for.
            vel: The spatial velocity to set, where the linear component is in the first 3 elements and the angular component is in the last 3 elements.
        """

        if body < 0 or body >= len(self.body_q):
            raise ValueError(f"Body index {body} is out of range")

        self.body_qd[body] = vel
